...
describe('sendShortAnswerRequest', () => {
  it('should return answer endpoint for question', async () => {
    const { data } = await sendShortAnswerRequest(validAppId, 'What is the capital of Berlin?')
  ...
  it('should return answer endpoint for solve', async () => {
    const { data } = await sendShortAnswerRequest(validAppId, 'solve(x^2 / 20 = 2400^4, x)')
